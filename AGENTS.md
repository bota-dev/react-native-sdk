...
3. **Test against physical device**: link into demo app (see Local Testing below) and run the affected flow:
   ...
   - **Factory-reset changes** — verify grant-gated opcode `0x06`, persist the three-byte result before writing explicit receipt opcode `0x0A`, wait for one-byte `RESET_FINALIZED` before backend finalization, report `RESET_PENDING` distinctly during provisioning, replay after disconnect-before-receipt, authenticated backend finalization retry, and complete local-recording removal per [`Device-Provisioning §3.1`](../internal-docs/device/Device-Provisioning.md#31-authenticated-factory-reset)
...
boundary and on failure. Matching signed-blob results additionally log only the
document kind and numeric device result. Preserve the original error for callers,
but never add provider payloads, signed URLs, documents, write IDs or nonce bytes
to these diagnostic logs. Phase entry is not completion evidence.
...
| [Connection Management](../internal-docs/device/Connection-Management.md) ([中文](../internal-docs/device/Connection-Management_ZH.md)) | Partner-facing WiFi/4G heartbeat, state reporting, dynamic policy and direct-upload workflow | Target; v0x02 remains compatibility behavior and conformance is tracked in System Design v5 |
...
| [Remote Device Control](../internal-docs/device/Remote-Recording-Control.md) ([中文](../internal-docs/device/Remote-Recording-Control_ZH.md)) | One command identity across App/BLE and direct delivery, exact authorization, receipt/result and recovery | Authoritative target; released recording-scope Grant remains compatibility behavior |
