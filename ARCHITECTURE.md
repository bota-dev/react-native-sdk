...
confirmation). The failure log contains only a static phase label; matching
signed-blob responses add only the document kind and numeric device result. No
provider error payloads, signed documents, write IDs, nonce values or storage URLs
are logged. The original error still reaches the caller; a phase log is not proof
