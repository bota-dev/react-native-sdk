...
- `DeviceManager.bleFactoryReset(device, grantBlob, persistResult)` — sends opcode `0x06`, requires the exact three-byte wipe-success result, awaits the caller's durable `persistResult` callback, subscribes for finalization, then writes DEVICE_COMMAND `0x0A`. It resolves only after firmware returns one-byte `RESET_FINALIZED` (`0x07`) following durable local project-state clear. A malformed result, rejected persistence callback, failed receipt write, or missing final confirmation leaves backend finalization retryable.
...
- Provisioning result `RESET_PENDING` (`0x06`) is not a token storage failure. `provision()` throws `ProvisioningError` code `RESET_PENDING` so callers keep the device powered, reconnect, and finish the prior reset before binding.
